...
require 'digest'
require 'mongo_jira/config_exception'
...
module MongoJira
  class Config
    ...
    #   MongoJira::Config::check()
    #   MongoJira::Config::check("/tmp/.jira.json")
    #
    ...
    def self.save(config, filename)
      cpy = config.clone
      if config[:password]
        cpy[:password] = MongoJira::Config::encrypt('password',cpy[:password])
      else
        cpy.delete(:password)
      end
      cpy[:context_path] ||= ''
    ...
    def initialize(file)
      ...
      config[:context_path] = ''
      config[:password] = MongoJira::Config::decrypt("password", config[:password]) if config[:password]
    end
    ...
    # @since 0.0.1
    def save
      MongoJira::Config::save(config,filename)
    end
